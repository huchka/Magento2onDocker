...
docker exec -it ${PHP} php /var/www/html/bin/magento setup:install \
  --db-host ${DATABASE} --db-name magento --db-user root --db-password root --timezone 'Asia/Tokyo' --currency JPY --use-rewrites 1 --cleanup-database \
  --backend-frontname admin --admin-firstname AdminFirstName --admin-lastname AdminLastName --admin-email admin@localhost \
